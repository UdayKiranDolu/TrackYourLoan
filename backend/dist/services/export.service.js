...
class ExportService {
     ...
     */
    static async getLoansForExport(filters) {
        const query = {};
        if (filters.userId) {
        ...
        return models_1.Loan.find(query)
     ...
     */
    static drawLine(doc, y, startX = 50, endX = 545, color = COLORS.border, lineWidth = 1) {
        doc
            .strokeColor(color)
            .lineWidth(lineWidth)
            .moveTo(startX, y)
     ...
     */
    static drawStatusBadge(doc, status, x, y, width = 70, height = 18) {
     ...
     */
    static drawCompactCard(doc, x, y, width, height, label, value, accentColor = COLORS.primary) {
        // Card background
        doc
            .roundedRect(x, y, width, height, 4)
     ...
     */
    static drawInfoRow(doc, x, y, label, value, labelWidth = 100, valueColor = COLORS.text) {
        doc
            .fontSize(9)
            .font('Helvetica')
     ...
     */
    static async generatePDF(loans, res) {
        ...
        res.setHeader('Content-Disposition', `attachment; filename=loans_report_${Date.now()}.pdf`);
        doc.pipe(res);
     ...
     */
    static async generateLoanDetailPDF(loanId, userId, res) {
        const query = { _id: new mongoose_1.Types.ObjectId(loanId) };
        if (userId) {
            query.ownerUserId = new mongoose_1.Types.ObjectId(userId);
        }
        const loan = await models_1.Loan.findOne(query).populate('ownerUserId', 'email');
        if (!loan) {
            throw new errorHandler_middleware_1.AppError('Loan not found', 404);
        }
        const history = await models_1.LoanHistory.find({ loanId: new mongoose_1.Types.ObjectId(loanId) })
            .sort({ changedAt: -1 })
            .limit(5)
        ...
        res.setHeader('Content-Type', 'application/pdf');
        res.setHeader('Content-Disposition', `attachment; filename=loan_${loan.borrowerName.replace(/\s+/g, '_')}_${Date.now()}.pdf`);
        doc.pipe(res);
        const ownerEmail = loan.ownerUserId && typeof loan.ownerUserId === 'object'
            ? loan.ownerUserId.email
            : null;
            ...
            for (let i = 0; i < history.length && i < 5; i++) {
                ...
                for (let j = 0; j < entry.changes.length; j++) {
                    ...
                    if (change.field === 'dueDate') {
                        oldVal = this.formatDate(new Date(change.oldValue));
                        newVal = this.formatDate(new Date(change.newValue));
                        fieldLabel = 'Due Date';
                    }
                    else {
                        oldVal = this.formatCurrency(change.oldValue);
                        newVal = this.formatCurrency(change.newValue);
                        fieldLabel = 'Interest';
                    }
                    doc.fontSize(8).font('Helvetica').fillColor(COLORS.text);
                    if (j === 0) {
