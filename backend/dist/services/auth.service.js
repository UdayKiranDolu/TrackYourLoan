...
class AuthService {
    static async register(email, password) {
        ...
        if (existingUser) {
            throw new errorHandler_middleware_1.AppError('Email already registered', 409, 'EMAIL_EXISTS');
        }
        // Hash password and create user
    ...
    static async login(email, password) {
        // Validate input
        if (!email || !password) {
            throw new errorHandler_middleware_1.AppError('Email and password are required', 400, 'MISSING_FIELDS');
        }
        // Find user
        const user = await models_1.User.findOne({ email: email.toLowerCase() });
        if (!user) {
            throw new errorHandler_middleware_1.AppError('Invalid email or password', 401, 'INVALID_CREDENTIALS');
        }
        // Check if account is active (if you have status field)
        if (user.status === 'INACTIVE' || user.status === 'DISABLED') {
            throw new errorHandler_middleware_1.AppError('Your account has been disabled. Please contact support.', 403, 'ACCOUNT_DISABLED');
        }
        // Verify password
    ...
    static async refresh(refreshToken) {
        if (!refreshToken) {
            throw new errorHandler_middleware_1.AppError('Refresh token is required', 400, 'MISSING_TOKEN');
        }
        // Verify token
        let decoded;
        try {
            ...
            throw new errorHandler_middleware_1.AppError('Invalid refresh token', 401, 'INVALID_REFRESH_TOKEN');
        }
        // Find user and verify stored token
    ...
    static async logout(userId) {
        await models_1.User.findByIdAndUpdate(userId, { refreshToken: null });
    }
    static async changePassword(userId, currentPassword, newPassword) {
        const user = await models_1.User.findById(userId);
        if (!user) {
            throw new errorHandler_middleware_1.AppError('User not found', 404, 'USER_NOT_FOUND');
        }
        // Verify current password
        const isValid = await (0, password_1.comparePassword)(currentPassword, user.passwordHash);
        if (!isValid) {
            throw new errorHandler_middleware_1.AppError('Current password is incorrect', 400, 'INVALID_CURRENT_PASSWORD');
        }
        // Update password
